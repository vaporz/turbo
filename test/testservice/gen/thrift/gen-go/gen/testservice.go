...
type MinionsService interface {
  // Parameters:
  //  - Food
  Eat(food string) (r *EatResponse, err error)
}
...
func NewMinionsServiceClientFactory(t thrift.TTransport, f thrift.TProtocolFactory) *MinionsServiceClient {
  return &MinionsServiceClient{Transport: t,
    ProtocolFactory: f,
    InputProtocol: f.GetProtocol(t),
    OutputProtocol: f.GetProtocol(t),
    SeqId: 0,
...
// Parameters:
//  - Food
func (p *MinionsServiceClient) Eat(food string) (r *EatResponse, err error) {
  if err = p.sendEat(food); err != nil { return }
  return p.recvEat()
}

func (p *MinionsServiceClient) sendEat(food string)(err error) {
  oprot := p.OutputProtocol
  if oprot == nil {
    oprot = p.ProtocolFactory.GetProtocol(p.Transport)
    p.OutputProtocol = oprot
  ...
  p.SeqId++
  if err = oprot.WriteMessageBegin("Eat", thrift.CALL, p.SeqId); err != nil {
      return
  }
...
func (p *MinionsServiceClient) recvEat() (value *EatResponse, err error) {
  iprot := p.InputProtocol
  if iprot == nil {
    iprot = p.ProtocolFactory.GetProtocol(p.Transport)
    p.InputProtocol = iprot
  ...
  if method != "Eat" {
    err = thrift.NewTApplicationException(thrift.WRONG_METHOD_NAME, "Eat failed: wrong method name")
    return
  }
  if p.SeqId != seqId {
    err = thrift.NewTApplicationException(thrift.BAD_SEQUENCE_ID, "Eat failed: out of sequence response")
    return
  }
  if mTypeId == thrift.EXCEPTION {
  ...
  if mTypeId != thrift.REPLY {
    err = thrift.NewTApplicationException(thrift.INVALID_MESSAGE_TYPE_EXCEPTION, "Eat failed: invalid message type")
    return
  }
...
func (p *MinionsServiceProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
  name, _, seqId, err := iprot.ReadMessageBegin()
  if err != nil { return false, err }
  if processor, ok := p.GetProcessorFunction(name); ok {
    return processor.Process(seqId, iprot, oprot)
...
func (p *minionsServiceProcessorEat) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
  args := MinionsServiceEatArgs{}
  if err = args.Read(iprot); err != nil {
    iprot.ReadMessageEnd()
    x := thrift.NewTApplicationException(thrift.PROTOCOL_ERROR, err.Error())
...
var retval *EatResponse
  var err2 error
  if retval, err2 = p.handler.Eat(args.Food); err2 != nil {
